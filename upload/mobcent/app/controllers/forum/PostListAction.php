...
class PostListAction extends MobcentAction
{
    ...
    protected function getResult($params = array())
    {
        ...
        $app->loadForum($topic['fid'], $topic['tid']);//加载论坛参数，设置$_G['forum']参数
        global $_G;
        $forumInfo = $_G['forum'];
        // 检查权限
        global $_G;
        if (empty($_G['forum']['allowview']))//是否在权限表里设置
        {
    ...
    private function _getTopicInfo($topic, $params = array())
    {
        ...
        if (!empty($topic))
        {
            ...
            error_log('11111111111111111'.$post['subject']);
            if (!empty($post))
            {
                if ($editPerm['bbcode'])
                {
                ...
                error_log('2222222222222222222'.$content);
...
//                 $topicInfo['zanList'] = (array)DzForumThread::getZanList($tid);
                $postContent = $this->_getPostContent($content);//$content['main']是个array,元素有content及type
                error_log('3333333333333333333333'.$postContent);
